import pytest
from csvparsing import football_v1 as fb
...
def test_parse_next_line(mock_csv_data, mock_csv_file):
    all_lines = [line for line in fb.parse_next_line(mock_csv_data)]
    assert len(all_lines) == 2
    for line in all_lines:
        assert len(line) == 7
    with pytest.raises(TypeError) as te:
        next(fb.parse_next_line(42.0))
    all_lines_io = list(fb.parse_next_line(open(mock_csv_file)))
    assert len(all_lines_io) == 2
    for line in all_lines_io:
        assert len(line) == 7
